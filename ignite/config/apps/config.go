...
type Config struct {
	...
	// Ignite Apps are implemented as apps.
	Apps []App `yaml:"apps"`
}
...
func (p App) HasPath(path string) bool {
	...
	appPath := p.CanonicalPath()
	path = strings.Split(path, "@")[0]
	return appPath == path
}

// CanonicalPath returns the canonical path of a app (excludes version ref).
func (p App) CanonicalPath() string {
...
func (c *Config) Save() error {
	errf := func(err error) error {
		return errors.Errorf("app config save: %w", err)
	}
	if c.path == "" {
